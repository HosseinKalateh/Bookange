    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header p-0">
                <h3 class="card-title">{{ __('ListTitle', ['name' => __(\Illuminate\Support\Str::plural('Translator')) ]) }}</h3>

                <div class="px-2 mt-4">

                    <ul class="breadcrumb mt-3 py-3 px-4 rounded" style="background-color: #e9ecef!important;">
                        <li class="breadcrumb-item"><a href="@route(getRouteName().'.home')" class="text-decoration-none">{{ __('Dashboard') }}</a></li>
                        <li class="breadcrumb-item active">{{ __(\Illuminate\Support\Str::plural('Translator')) }}</li>
                    </ul>

                    <div class="row justify-content-between mt-4 mb-4">
                        @if(getCrudConfig('translator')->create)
                        <div class="col-md-4 right-0">
                            <a href="@route(getRouteName().'.translators.create')" class="btn btn-success">{{ __('CreateTitle', ['name' => __('Translator') ]) }}</a>
                        </div>
                        @endif
                        @if(getCrudConfig('translator')->searchable())
                        <div class="col-md-4">
            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-hover">
                    ...
                    @foreach($translators as $translator)
                        @livewire('admin.translator.single', ['translator' => $translator], key($translator->id))
                    @endforeach
                    </tbody>
                </table>
            </div>
